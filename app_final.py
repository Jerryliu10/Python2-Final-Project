...
# Transfer fips into 5 digit string
...
# -------------------------------------------------------------------------------------
# App layout
app = dash.Dash(__name__) # this was introduced in Dash version 1.12.0    prevent_initial_callbacks=True
...
#    print("Complete data of all selected cells: {}".format(slctd_cell))
    ...
    if "state" in dff and "cases" in dff:           # Set county and case
        return [
            dcc.Graph(id='bar-chart',
                      figure=px.bar(
                          ...
                          x="state",
                          y='cases',
                          labels={"number of cases"}   # need to revise!  : "% of Pop took online course"
...
rate = round(df["deaths"]/df["cases"]*100, 2)
rate = rate.fillna(0).tolist()
...
def update_map(all_rows_data, slctd_row_indices):
    ...
    # highlight selected counties on map
    borders = [5 if i in slctd_row_indices else 1
               for i in range(len(dff))]
    
    if "fips" in dff and "cases" in dff:
        return [
            dcc.Graph(id='choropleth',
                      style={'height': 700},
                      figure=ff.create_choropleth(
                      fips=fips, values=rate,
                      show_state_data=False,
                      show_hover=True, centroid_marker={'opacity': 0},
                      title='Covid Death Rate',
                      legend_title='% death rate').update_layout(showlegend=False, title=dict(font=dict(size=28), x=0.5, xanchor='center')))
        ]
